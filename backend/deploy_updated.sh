...
# This preserves the existing admin system and adds public API endpoints
...
if pgrep -f "gunicorn.*app:app" > /dev/null; then
    ...
    curl -s http://localhost:8000/api/bookings?limit=3 | python3 -c "import sys, json; data=json.load(sys.stdin); print(f\"✓ Found {data.get('count', 0)} bookings\"); [print(f\"  - {b.get('name')} ({b.get('phone')})\") for b in data.get('bookings', [])[:3]]"
    echo ""
    echo ""
